...
if __name__ == "__main__":
  ...
  for line in f:
    ...
    if check_test:
      if 'Test net output' in line and 'loss_full' in line:
    ...
    # add for heatmap
    if 'Train net output #1: loss_fusion' in line:
      training_fusion_loss.append(float(line.strip().split(' ')[-2]))
    #if 'Train net output #: loss_heatmap' in line:
      #training_heatmap_loss.append(float(line.strip().split(' ')[-2]))
